...
export async function get_results_ordered(gameID) {
  return get_results(gameID).then((res) => {
    ...
    for (let i = 0; i < res.length; i++) {
      results.push([res[i].name, res[i].score]);
    }
    results.sort((a, b) => b);
...
// Get quiz info (title and number of players)
export async function get_quiz_info(gameID) {
  try {
    const response = await axios.post(`${host}/get_quiz_info`, {
      game_id: gameID,
    });
    return response.data;
  } catch (error) {
...
// Get quiz questions by gameID
export async function get_quiz_questions(gameID) {
  try {
